...
function getYesterdayCommits(cwd: string): string[] {
  try {
    // Get commits from last 24 hours by the current git user
    const author = execSync("git config user.name", { cwd })
      .toString()
      .trim();

    const raw = execSync(
      `git log --since="24 hours ago" --author="${author}" --pretty=format:"%s" --no-merges`,
      { cwd }
    )
      .toString()
      .trim();
...
function getWebviewContent(standup: string, commits: string[]): string {
...
export function activate(context: vscode.ExtensionContext) {
  const disposable = vscode.commands.registerCommand(
    "standup-writer.generate",
    async () => {
      // 1. Check workspace
      const workspacePath = getWorkspacePath();
      if (!workspacePath) {
        vscode.window.showErrorMessage(
          "Standup Writer: Please open a project folder first."
        );
        return;
      }
      ...
      if (!isGitRepo(workspacePath)) {
        vscode.window.showErrorMessage(
          "Standup Writer: This folder is not a git repository."
        );
        return;
      }
      ...
      if (!apiKey) {
        ...
        if (!apiKey) {
          vscode.window.showWarningMessage(
            "Standup Writer: API key is required. Add it in Settings → Standup Writer."
          );
          return;
        }
      ...
      if (commits.length === 0) {
        vscode.window.showWarningMessage(
          "Standup Writer: No commits found in the last 24 hours. Make sure you have commits today or yesterday."
        );
        return;
      }
            ...
            panel.webview.onDidReceiveMessage((message) => {
              if (message.command === "copied") {
                vscode.window.showInformationMessage(
                  "✅ Standup copied to clipboard! Go paste it in Slack/Teams."
                );
              }
            });
          } catch (err: any) {
            vscode.window.showErrorMessage(
              `Standup Writer: ${err?.message ?? "Something went wrong. Check your API key."}`
            );
          }
        }
      );
